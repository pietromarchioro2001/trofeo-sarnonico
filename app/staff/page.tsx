...
export default function StaffPage() {
  const { isStaffMode, enableAccess, disableAccess } = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error || !data) {
        ...
        if (rememberMe) localStorage.setItem('staffCode', code.trim().toUpperCase());
        else localStorage.removeItem('staffCode');
    ...
    return (
      <div className="min-h-screen bg-[#F5F5F7] flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">Sei già loggato come Staff.</p>
          <button onClick={() => router.push('/')} className="w-full bg-[#581C24] text-white font-bold py-3 rounded-lg mb-3">Vai alla Home</button>
    ...
    <div className="min-h-screen bg-[#F5F5F7] flex flex-col">
      <div className="relative h-40 sm:h-48 w-full overflow-hidden flex-shrink-0">
        ...
        <div className="absolute inset-0 flex items-start justify-center pt-6">
          <h1 className="text-3xl font-black text-white uppercase tracking-wider drop-shadow-2xl font-oswald">AREA STAFF</h1>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
          <div className="text-center mb-6">
            <div className="w-16 h-16 bg-[#581C24]/10 rounded-full flex items-center justify-center mx-auto mb-3">
              <svg className="w-8 h-8 text-[#581C24]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
            </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="password" value={code} onChange={(e) => setCode(e.target.value)} placeholder="Inserisci codice" className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#581C24] text-sm" />
            </div>
